# =============================================================================
#         Kaggle 持久化媒体处理服务 - 主应用 v2.0 (方案2 - 优化版)
# =============================================================================
#
# 功能:
...
# --- Web 框架与 HTTP 客户端 ---
# 注意：Flask 的实例化和路由定义被移动到了 main 函数内部
...
# --- 字幕提取相关库 (将在 main 函数中按需导入或验证) ---
# 注意：这些库的重量级操作（如模型加载）将在 main 中精确控制
try:
    import torch
    import torchaudio
...
denoiser_model_global = None # 用于存储预加载的降噪模型
FRP_SERVER_ADDR = None # 将在解密后填充

# =============================================================================
# --- 第 2 步: 解密与配置加载模块 (业务逻辑函数) ---
# =============================================================================
...
# =============================================================================
# --- 第 3 步: 环境检查、辅助函数与 Killer API (业务逻辑函数) ---
# =============================================================================
    ...
    for i in range(num_chunks):
        ...
        try:
            ...
            vad_parameters = {
                ...
                "max_speech_duration_s": 10.0, # 稍微放宽以容纳长句
...
def process_unified_task(task_id: str, params: dict):
    ...
    try:
        ...
        upload_tasks = []
        # 使用字典来追踪 future 对应的任务类型
        future_map = {}
        with ThreadPoolExecutor(max_workers=2) as executor:
            # 提交视频上传任务
            if params["upload_video"]:
                future = executor.submit(api_client.upload_file, str(local_file_path))
            ...
            if params["upload_subtitle"] and srt_content:
                ...
                future = executor.submit(api_client.upload_file, str(srt_path))
                future_map[future] = "subtitle"
        
        # 处理上传结果
        for future in future_map:
            task_type = future_map[future]
            target_result = final_result["video_file"] if task_type == "video" else final_result["subtitle_file"]
            ...
            try:
                ...
                log_system_event("error", f"任务 {task_id} 的 {task_type} 文件上传失败: {e}")
                target_result["status"] = "upload_failed"
                target_result["details"] = str(e)
...
def main():
    ...
    try:
        ...
        # =====================================================================
        # --- 阶段 2: 加载 AI 模型 (内存/GPU 峰值) ---
        # =====================================================================
        log_system_event("info", "服务正在启动：[2/7] 准备 AI 模型...")
        ...
        log_system_event("info", "  -> 等待 5 秒让系统 I/O 稳定...")
        time.sleep(5)
        
        # 现在加载模型，这是内存和 GPU 消耗的峰值点
        preload_models()
        
        # =====================================================================
        # --- 阶段 3: 导入 Web 框架并定义路由 (增加常驻内存) ---
        # =====================================================================
        log_system_event("info", "服务正在启动：[3/7] 正在初始化 Web 框架...")
        ...
        def unified_upload_endpoint():
            ...
            with tasks_lock:
                tasks[task_id] = {"task_id": task_id, "status": "pending", "stage": "queue", "details": "任务已创建，等待处理", "progress": 0, "result": None}
            threading.Thread(target=process_unified_task, args=(task_id, request_params)).start()
        ...
        def get_task_status_endpoint(task_id):
            ...
            else: return jsonify({"error": "未找到指定的 task_id"}), 404

        @app.route('/killer_status_frp', methods=['GET'])
        def health_status_endpoint():
            return jsonify({"status": "ok", "message": "统一媒体处理 API 服务正在运行，并通过 FRP 暴露。", "timestamp": time.time()}), 200
        ...
        def force_shutdown_endpoint():
            ...
            return jsonify({"status": "shutdown_initiated", "message": "关闭信号已接收。后台正在执行清理和内核关闭操作。"}), 200

        # =====================================================================
        # --- 阶段 4: 初始化和解密配置 ---
        # =====================================================================
        log_system_event("info", "服务正在启动：[4/7] 正在解密和加载配置...")
        ...
        # =====================================================================
        # --- 阶段 5: 启动后台进程 (增加常驻内存和 CPU) ---
        # =====================================================================
        log_system_event("info", "服务正在启动：[5/7] 正在初始化 MixFile 客户端...")
        api_client_base_url = f"http://127.0.0.1:{MIXFILE_LOCAL_PORT}"
        api_client = MixFileCLIClient(base_url=api_client_base_url)

        log_system_event("info", "服务正在启动：[6/7] 正在启动后台服务...")
        
        # 启动 MixFileCLI (JVM)
        log_system_event("info", "  -> 启动 MixFileCLI...")
