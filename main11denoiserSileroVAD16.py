...
#             speech_pad_ms=200
#         )
#         log_system_event("info", f"✅ VAD分析完成，共发现 {len(global_speech_timestamps)} 个潜在语音片段。", in_worker=True)
#     except Exception as e:
#         log_system_event("error", f"VAD处理失败: {e}", in_worker=True)
...
#     for speech in global_speech_timestamps:
...
def preprocess_audio_for_subtitles(
    ...
    """
    [最终正确版] 此版本通过彻底移除有问题的AI降噪步骤，
    回归到“单一文件源”的健壮架构。VAD直接处理原始音频，
    并从同一原始音频进行切割，从根本上杜绝所有已知问题。
    """
    import torch
    import torchaudio
...
def main():
    ...
    try:
        ...
        # 【核心修改】第二步：安装其余的库
        install_other_cmd = "pip install -q pydantic pydub faster-whisper@https://github.com/SYSTRAN/faster-whisper/archive/refs/heads/master.tar.gz denoiser google-generativeai requests psutil onnxruntime silero"        
